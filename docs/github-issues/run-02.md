# Experiment candidate run-02

## Mục tiêu

Thực hiện AI candidate run-02 theo prompt mơ hồ đã freeze và bàn giao bằng một
PR độc lập.
...
- [ ] Có `candidates/run-02/` đầy đủ.
- [ ] Có `experiments/transcripts/run-02.response.txt` nguyên trạng.
- [ ] Có một dòng run-02 trong `experiments/metadata.csv`.
- [ ] Có packaging patch riêng nếu cần; không có patch logic.
- [ ] Có checksum raw transcript.
- [ ] Đã mở PR branch `candidate/run-02`.
